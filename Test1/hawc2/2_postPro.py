...
vf=np.array([1.5e6 ,1e4 ,1e5 ,1e6 ,1e7 ,2.5e6 ,2e4 ,2e5 ,2e6 ,3e6 ,4e4 ,4e5 ,4e6 ,6e4 ,6e5 ,6e6 ,8e4 ,8e5 ,8e6] )
vf.sort()
...
if 1 in IPostPro:
    ...
    dfOut = pd.DataFrame(columns=cols, data=data)
    dfOut.to_csv('tiploads.csv', index=False, sep='\t')
